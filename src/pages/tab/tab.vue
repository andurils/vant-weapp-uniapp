<template>
	<view class="app">
		...
		<wrap title="切换动画"></wrap>
		<van-tabs animated>
			<van-tab v-for="(item, index) in tabs" :key="index" :title="'标签' + item">
				<view class="content" style="height: 100px;">{{ '内容' + item }}</view>
			</van-tab>
		</van-tabs>

		<wrap title="滑动切换"></wrap>
		<van-tabs swipeable>
			<van-tab v-for="(item, index) in tabs" :key="index" :title="'标签' + item">
				<view class="content">{{ '内容' + item }}</view>
			</van-tab>
